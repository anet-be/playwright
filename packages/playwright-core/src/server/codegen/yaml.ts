...
// Scenario metadata defaults
const SCENARIO_VERSION = '0.2';
const DEFAULT_SCENARIO_NAME = 'Recorded Scenario';
const GENERATED_FILE_HEADER = '# Generated by Genfest';

// Password detection and masking
const PASSWORD_PATTERN = /password|pwd|secret/i;
const PASSWORD_ENV_VAR = '${env:PASSWORD}';

// Debug configuration
const DEBUG_ENV_VAR = 'GENFEST_HTML_DIR';
...
function matchText(t: TextMatcher): boolean {
  if (typeof t === 'string')
    return PASSWORD_PATTERN.test(t);
  if ('value' in t && typeof t.value === 'string' && PASSWORD_PATTERN.test(t.value))
    return true;
  if ('pattern' in t && typeof t.pattern === 'string' && PASSWORD_PATTERN.test(t.pattern))
    return true;
  return false;
}
...
function maybeMaskValue(el: Element | undefined, text: string | undefined): string | undefined {
  ...
  if ('testId' in el && PASSWORD_PATTERN.test(el.testId))
    return PASSWORD_ENV_VAR;
  if ('label' in el && matchText(el.label))
    return PASSWORD_ENV_VAR;
  if ('placeholder' in el && matchText(el.placeholder))
    return PASSWORD_ENV_VAR;
  if ('role' in el && PASSWORD_PATTERN.test((el as any).name ?? ''))
    return PASSWORD_ENV_VAR;
  if ('text' in el && matchText(el.text))
    return PASSWORD_ENV_VAR;
  if ('css' in el && PASSWORD_PATTERN.test(el.css))
    return PASSWORD_ENV_VAR;
  if ('xpath' in el && PASSWORD_PATTERN.test(el.xpath))
    return PASSWORD_ENV_VAR;
...
function decodeModifiers(mask: number | undefined): Modifier[] | undefined {
  ...
  if (mask & MODIFIER_ALT)
    result.push('Alt');
  if (mask & MODIFIER_CONTROL)
    result.push('Control');
  if (mask & MODIFIER_META)
    result.push('Meta');
  if (mask & MODIFIER_SHIFT)
    result.push('Shift');
...
export class YamlLanguageGenerator implements LanguageGenerator {
  ...
  generateHeader(options: LanguageGeneratorOptions): string {
    ...
    this._meta.name = (this._seedUrl ? new URL(this._seedUrl).hostname : DEFAULT_SCENARIO_NAME);
